...
def start_simulation(data):
	...
	throughput = data[3]
	scheme = data[4]
	location = scheme+"_"+str(downloadSize)
	subprocess.check_call('mkdir %s'%(location),shell=True)
	subprocess.check_call('./waf --cwd=%s --command-template="%%s --X2LinkDelay=%s --BuildingNum=%s --SourceRate=%s" --run globecom2019_pbh' % (location,x2_delay, buildingNum, throughput),shell=True)
...
downloadSizeSet = ['64','128','256','512']
schemeSet = ['X2','Proxy','PBH']
whichScheme = 1
paramsSet = []
for  i in range(0,200,1):
	index = i+1
	for j in range(len(downloadSizeSet)):
		downloadSize = downloadSizeSet[j]
		for k in range(len(x2DelaySet)):
			x2Delay = x2DelaySet[k]
			throughput = throughputSet[0]
			scheme = schemeSet[whichScheme]
			params = [downloadSize,x2Delay,buildingNum,throughput,scheme]
