...
McUePdcp::BufferingAndReordering(Ptr<Packet> p){ // sjkang
	...
	if ((((receivedPDCP_SN -Last_Submitted_PDCP_RX_SN) > reorderingWindow) && (receivedPDCP_SN > Last_Submitted_PDCP_RX_SN))
			|| (((Last_Submitted_PDCP_RX_SN > receivedPDCP_SN) && ((Last_Submitted_PDCP_RX_SN-receivedPDCP_SN)< reorderingWindow))))
	{
		// if(firstOne ==1){
	...
	*/
	...
	if ((receivedPDCP_SN == Last_Submitted_PDCP_RX_SN +1)||(receivedPDCP_SN==Last_Submitted_PDCP_RX_SN-m_maxPdcpSn) )
	{
		...
		for (int c = 0 ; c< p; c++){
			...
			if(Last_Submitted_PDCP_RX_SN == Reordering_PDCP_RX_COUNT % MAX_PDCP_SN + 1)
				check =true;
			if(Last_Submitted_PDCP_RX_SN ==m_maxPdcpSn && Reordering_PDCP_RX_COUNT % (MAX_PDCP_SN+1)==0)
				check =true;
	...
	if (!t_ReorderingTimer.IsRunning())
	{
		if (PdcpBuffer.size()>=1){
			...
			Reordering_PDCP_RX_COUNT = Next_PDCP_RX_SN + present_RX_HFN* (MAX_PDCP_SN+1);
