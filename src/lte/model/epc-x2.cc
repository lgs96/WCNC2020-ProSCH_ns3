...
namespace ns3 {
			...
			if (procedureCode == EpcX2Header::HandoverPreparation)
			{
				...
				if (messageType == EpcX2Header::InitiatingMessage)
				{
					if(m_isMinimum && !m_isMmWave)
					{
						...
						packet->PeekHeader (x2HoReqHeader);
						packet->AddHeader (x2Header);

						uint16_t relayTargetCell = x2HoReqHeader.GetTargetCellId ();

						Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [relayTargetCell];
					...
					else
					{
						...
						NS_LOG_UNCOND (Simulator::Now()<<"Recv X2 message: HANDOVER REQUEST");
						...
						// revised for MCT, gsoul
						params.sourceCellId   = x2HoReqHeader.GetSourceCellId ();
				...
				else if (messageType == EpcX2Header::SuccessfulOutcome)
				{
					if(m_isMinimum && !m_isMmWave)
					{
						...
						packet->PeekHeader (x2HoReqAckHeader);
						packet->AddHeader (x2Header);

						uint16_t relayCell = x2HoReqAckHeader.GetSourceCellId ();

						Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [relayCell];

						Ptr<Socket> sourceSocket;
						sourceSocket = socketInfo->m_localCtrlPlaneSocket;
						Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
					...
					else
					{
						...
						NS_LOG_UNCOND (Simulator::Now()<<"Recv X2 message: HANDOVER REQUEST ACK");
						...
						params.sourceCellId   = x2HoReqAckHeader.GetSourceCellId();
						params.targetCellId = x2HoReqAckHeader.GetTargetCellId();
				...
				else if (messageType == EpcX2Header::SuccessfulOutcomeToLte)
				{	
					NS_LOG_LOGIC ("Recv X2 message: HANDOVER REQUEST ACK");
					...
					NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId);
					NS_LOG_LOGIC ("newEnbUeX2apId = " << params.newEnbUeX2apId);
					NS_LOG_UNCOND ("sourceCellId = " << params.sourceCellId);
					NS_LOG_UNCOND ("targetCellId = " << params.targetCellId);
					m_x2SapUser->RecvHandoverRequestAckFromLte (params);
				}
	...
	void
		EpcX2::DoSetRelayNode (uint16_t relayCellId)
		{
			NS_LOG_FUNCTION (this);

			NS_LOG_LOGIC ("relay Cell Id = " << relayCellId);
			m_relayCellId = relayCellId;
			m_isMmWave = true;
		}
	...
	void
		EpcX2::DoSendHandoverRequest (EpcX2SapProvider::HandoverRequestParams params)
		{
			...
			x2HoReqHeader.SetSourceCellId (params.sourceCellId);
	...
	void
		EpcX2::DoSendHandoverRequestAck (EpcX2SapProvider::HandoverRequestAckParams params)
		{
			...
			{
				localSocket = m_x2InterfaceSockets [m_relayCellId]->m_localCtrlPlaneSocket;
				remoteIpAddr = m_x2InterfaceSockets [m_relayCellId]->m_remoteIpAddr;
			}
			...
			// gsoul, for relaying
			x2HoAckHeader.SetSourceCellId (params.sourceCellId);
			x2HoAckHeader.SetTargetCellId (params.targetCellId);
	...
	void
		EpcX2::DoSendHandoverRequestAckToLte (EpcX2SapProvider::HandoverRequestAckParams params)
		{
			...
			EpcX2Header x2Header;
			x2Header.SetMessageType (EpcX2Header::SuccessfulOutcomeToLte);
			x2Header.SetProcedureCode (EpcX2Header::HandoverPreparation);
			x2Header.SetLengthOfIes (x2HoAckHeader.GetLengthOfIes ());
			x2Header.SetNumberOfIes (x2HoAckHeader.GetNumberOfIes ());
