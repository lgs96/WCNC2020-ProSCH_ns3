...
double tcp_throughput[100];

	static void
Reset_throughput(Ptr<OutputStreamWrapper> stream, uint16_t i)
{
	std::cout<<"Simulator time: "<<Simulator::Now().GetSeconds()<<" for "<<i+1<<std::endl;
	*stream->GetStream () << Simulator::Now().GetSeconds()<<"\t"<<tcp_throughput[i]/1e5<<std::endl;	
	tcp_throughput[i] = 0;
	Simulator::Schedule (MilliSeconds(100), &Reset_throughput,stream,i);
}
	static void
TxChange (Ptr<OutputStreamWrapper> stream, uint16_t i, const Ptr<const Packet> packet, const TcpHeader &header, const Ptr<const TcpSocketBase> socket)
{
	tcp_throughput[i] += 1454*8;
}
	static void
...
*/
	...
	Config::ConnectWithoutContext (pathTx.str ().c_str (), MakeBoundCallback(&TxChange, stream6, nodeNum));
}
...
main (int argc, char *argv[])
{
	...
	//	Config::SetDefault ("ns3::LteRlcAm::MaxTxBufferSize", UintegerValue (10*1024));
	...
	//	Config::SetDefault ("ns3::LteRlcUmLowLat::MaxTxBufferSize", UintegerValue (1024 * 1024));
		Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (1024*100));
	...
	Config::SetDefault ("ns3::LteRlcAm::MaxTxBufferSize", UintegerValue (1024));
	...
	remoteHostContainer.Create (ueNum);
	Ptr<Node> remoteHost;
	InternetStackHelper internet;
	internet.Install (remoteHostContainer);
	Ipv4Address remoteHostAddr[ueNum];
...
*/
	...
	uemobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
			...
			"Speed",StringValue("ns3::UniformRandomVariable[Min=6.0|Max=14.0]"));
	uemobility.Install (ueNodes);
...
//	Simulator::Schedule (Seconds (22), &ChangeSpeed, ueNodes.Get (0), Vector (0, 0, 0));
	...
	for(uint32_t u = 0; u<ueNodes.GetN();++u){
		if(tcp)
		{
			...
			app->Setup (ns3TcpSocket, sinkAddress, 1400, 0xffffffff, DataRate ("90Mb/s"));

			remoteHostContainer.Get (u)->AddApplication (app);

			AsciiTraceHelper asciiTraceHelper;

			double ueStartTime = 2;

			std::ostringstream fileName_1;
			fileName_1<<"bb-Tcp_Window_"<<Tcp<<"_node"<<u<<".txt";
			...
			fileName_2<<"bb-Tcp_Rtt_"<<Tcp<<"_node"<<u<<".txt";
			...
			fileName_3<<"bb-Tcp_Throughput_"<<Tcp<<"_node"<<u<<".txt";
			Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper.CreateFileStream (fileName_3.str().c_str());
			Simulator::Schedule (Seconds(1+u*ueStartTime),&CalculateThroughput,stream2,sinkApps.Get(u)->GetObject<PacketSink>(),u);

			std::ostringstream fileName_4;
			fileName_4<<"bb-Tcp_Ack" <<Tcp<<"_node"<< u <<".txt";
			...
			Simulator::Schedule (Seconds (0.1+u*ueStartTime),&Reset_ack,stream_4,u);

			std::ostringstream fileName_5;
			fileName_5<<"bb-Tcp_real" <<Tcp<<"_node"<< u <<".txt";
			AsciiTraceHelper asciiTraceHelper_5;
			Ptr<OutputStreamWrapper> stream_5 = asciiTraceHelper_5.CreateFileStream(fileName_5.str().c_str());
			Simulator::Schedule (Seconds (0.1+u*ueStartTime),&Reset_throughput,stream_5,u);

			Simulator::Schedule (Seconds (0.2001+u*ueStartTime), &Traces, u);
			...
			app->SetStartTime (Seconds (1+u*ueStartTime));
		...
		else
		{
			...
			std::ostringstream fileName_3;
			fileName_3<<"bb-Udp_Throughput_node"<<u<<".txt";
			Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper.CreateFileStream (fileName_3.str().c_str());
