...
namespace ns3 {
	...
	// Process8: Add function overloading for RRC connection reconfiguration msg
	void 
		MmWaveLteEnbRrcProtocolReal::DoSendRrcConnectionReconfiguration (uint16_t rnti, LteRrcSap::RrcConnectionReconfiguration msg)
		{
			...
			transmitPdcpSduParameters.rnti = rnti;
			transmitPdcpSduParameters.lcid = 1;
			transmitPdcpSduParameters.toLte = false;
			m_setupUeParametersMap[rnti].srb1SapProvider->TransmitPdcpSdu (transmitPdcpSduParameters);
		}
	// Process8: function overloading for RRC connection reconfiguration msg
	void
		MmWaveLteEnbRrcProtocolReal::DoSendRrcConnectionReconfiguration (uint16_t rnti, LteRrcSap::RrcConnectionReconfiguration msg, bool fromLte)
		{
			NS_LOG_FUNCTION(this<<"rnti: "<<rnti<<" from Lte? "<<fromLte);
			Ptr<Packet> packet = Create<Packet> ();
			...
			transmitPdcpSduParameters.rnti = rnti;
			transmitPdcpSduParameters.lcid = 1;
			transmitPdcpSduParameters.toLte = fromLte;
