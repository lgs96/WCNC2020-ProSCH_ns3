...
//	Config::SetDefault("ns3::McEnbPdcp::EnableDuplication", BooleanValue(isDuplication));
	...
	Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue (100*1024*1024));
...
//	allEnbNodes.Add(mmWaveEnbNodes_73G);
	...
	Vector mmw4Position = Vector(100.0, 0.0, 35); //73Ghz // path 1
	Vector mmw5Position = Vector(100.0,50.0, 35);  ///73Ghz //path 1
	Vector mmw6Position = Vector(100.0, 100, 35); //73Ghz // path 1
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if (dl)
			{
				...
				app->Setup (ns3TcpSocket, sinkAddress, 1400, 0xffffffff, DataRate ("3500Mbps"),isRandom);
