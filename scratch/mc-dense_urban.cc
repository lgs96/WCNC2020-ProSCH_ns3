...
//	mmWaveEnbNodes.Add(mmWave73GhzEnbNodes);
	...
	Vector mmw1_1Position = Vector(-300, 300, 35);  ///28Ghz
...
//	enbPositionAlloc->Add(mmw_73Position);
	...
	for (uint16_t i = 0; i<ueNodes.GetN(); i++) {
		if (i == 0)
			uePositionAlloc->Add(Vector(-300, 300, 10));
		else if (i == 1)
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if (dl)
			{
				...
				app->Setup (ns3TcpSocket, sinkAddress, 1400, 0xffffffff, DataRate ("4000Mbps"),isRandom);

				remoteHostContainer.Get (u)->AddApplication (app);

				double ueStartTime = 0;
				...
				Simulator::Schedule (Seconds (0.1+u*ueStartTime), &CalculateThroughput,stream_2,serverApps.Get(u)->GetObject<PacketSink>(),u);
				...
				Simulator::Schedule (Seconds (0.1+u*ueStartTime),&Reset_ack,stream_3,u);

				Simulator::Schedule (Seconds (0.2001+u*ueStartTime), &Traces, u);

				app->SetStartTime (Seconds (0.1+(u)*ueStartTime));
