...
static const std::string g_ueManagerStateName[UeManager::NUM_STATES] =
		{ "INITIAL_RANDOM_ACCESS", "CONNECTION_SETUP", "CONNECTION_REJECTED",
				"CONNECTED_NORMALLY", "CONNECTION_RECONFIGURATION",
				"CONNECTION_REESTABLISHMENT", "HANDOVER_PREPARATION",
				"HANDOVER_JOINING", "HANDOVER_PATH_SWITCH", "HANDOVER_LEAVING",
...
UeManager::UeManager(Ptr<LteEnbRrc> rrc, uint16_t rnti, State s) :
		m_lastAllocatedDrbid(0), m_rnti(rnti), m_imsi(0), m_lastRrcTransactionIdentifier(
				0), m_rrc(rrc), m_state(s), m_pendingRrcConnectionReconfiguration(
				false), m_sourceX2apId(0), m_sourceCellId(0), m_needPhyMacConfiguration(
				false), m_x2forwardingBufferSize(0), m_maxx2forwardingBufferSize(
				2 * 1024), m_allMmWaveInOutageAtInitialAccess(false) {
...
void UeManager::DoInitialize() {
	...
	// configure PHY
	m_rrc->m_cphySapProvider->SetTransmissionMode(m_rnti,
			m_physicalConfigDedicated.antennaInfo.transmissionMode);
	m_rrc->m_cphySapProvider->SetSrsConfigurationIndex(m_rnti,
			m_physicalConfigDedicated.soundingRsUlConfigDedicated.srsConfigIndex);
	...
	case HANDOVER_JOINING:
		m_handoverJoiningTimeout = Simulator::Schedule(
				m_rrc->m_handoverJoiningTimeoutDuration,
...
TypeId UeManager::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::UeManager").SetParent<Object>().AddConstructor<
					UeManager>().AddAttribute("DataRadioBearerMap",
					"List of UE DataRadioBearerInfo by DRBID.",
					ObjectMapValue(),
					MakeObjectMapAccessor(&UeManager::m_drbMap),
					MakeObjectMapChecker<LteDataRadioBearerInfo>()).AddAttribute(
					"DataRadioRlcMap", "List of UE Secondary RLC by DRBID.",
					ObjectMapValue(),
					MakeObjectMapAccessor(&UeManager::m_rlcMap),
					MakeObjectMapChecker<RlcBearerInfo>()).AddAttribute("Srb0",
					"SignalingRadioBearerInfo for SRB0", PointerValue(),
					MakePointerAccessor(&UeManager::m_srb0),
					MakePointerChecker<LteSignalingRadioBearerInfo>()).AddAttribute(
					"Srb1", "SignalingRadioBearerInfo for SRB1", PointerValue(),
					...
					MakeUintegerChecker<uint16_t>()).AddTraceSource(
					"StateTransition",
					...
					"ns3::UeManager::StateTracedCallback").AddTraceSource(
					"SecondaryRlcCreated",
					"fired upon receiving RecvRlcSetupRequest",
					MakeTraceSourceAccessor(
							&UeManager::m_secondaryRlcCreatedTrace),
...
std::pair<uint16_t, uint16_t> UeManager::GetSource(void) {
	return std::pair<uint16_t, uint16_t>(m_sourceCellId, m_sourceX2apId);
}

void UeManager::SetImsi(uint64_t imsi) {
	m_imsi = imsi;
}

void UeManager::SetIsMc(bool isMc) {
	m_isMc = isMc;
}
...
bool UeManager::GetIsMc() const {
	return m_isMc;
}

void UeManager::SetupDataRadioBearer(EpsBearer bearer, uint8_t bearerId,
		uint32_t gtpTeid, Ipv4Address transportLayerAddress) {
	...
	NS_ASSERT_MSG(bearerId == 0 || bid == bearerId,
			"bearer ID mismatch (" << (uint32_t) bid << " != " << (uint32_t) bearerId << ", the assumption that ID are allocated in the same way by MME and RRC is not valid any more");
	...
	if (m_state == HANDOVER_JOINING) {
		...
		std::pair<std::map<uint32_t, LteEnbRrc::X2uTeidInfo>::iterator, bool> ret =
				m_rrc->m_x2uTeidInfoMap.insert(
						std::pair<uint32_t, LteEnbRrc::X2uTeidInfo>(gtpTeid,
								x2uTeidInfo));
	...
	drbInfo->m_logicalChannelIdentity = lcid;
	drbInfo->m_logicalChannelConfig.priority = m_rrc->GetLogicalChannelPriority(
			bearer);
...
void UeManager::StartDataRadioBearers() {
	...
	for (std::list<uint8_t>::iterator drbIdIt = m_drbsToBeStarted.begin();
		...
		std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator drbIt =
...
void UeManager::ScheduleRrcConnectionReconfiguration() {
	...
	case PREPARE_MC_CONNECTION_RECONFIGURATION: {
		...
		msg.radioResourceConfigDedicated.physicalConfigDedicated.haveAntennaInfoDedicated =
				false;
		msg.radioResourceConfigDedicated.physicalConfigDedicated.haveSoundingRsUlConfigDedicated =
				false;
		msg.radioResourceConfigDedicated.physicalConfigDedicated.havePdschConfigDedicated =
				false;
		m_rrc->m_rrcSapUser->SendRrcConnectionReconfiguration(m_rnti, msg);
		RecordDataRadioBearersToBeStarted();
...
void UeManager::PrepareHandover(uint16_t cellId) {
	...
	case CONNECTED_NORMALLY: {
		...
		hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo.plmnIdentityInfo.plmnIdentity =
				m_rrc->m_sib1.cellAccessRelatedInfo.plmnIdentityInfo.plmnIdentity;
		...
		hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo.csgIndication =
				m_rrc->m_sib1.cellAccessRelatedInfo.csgIndication;
		hpi.asConfig.sourceSystemInformationBlockType1.cellAccessRelatedInfo.csgIdentity =
				m_rrc->m_sib1.cellAccessRelatedInfo.csgIdentity;
		LteEnbCmacSapProvider::RachConfig rc =
				m_rrc->m_cmacSapProvider->GetRachConfig();
		hpi.asConfig.sourceSystemInformationBlockType2.radioResourceConfigCommon.rachConfigCommon.preambleInfo.numberOfRaPreambles =
				rc.numberOfRaPreambles;
		hpi.asConfig.sourceSystemInformationBlockType2.radioResourceConfigCommon.rachConfigCommon.raSupervisionInfo.preambleTransMax =
				rc.preambleTransMax;
		hpi.asConfig.sourceSystemInformationBlockType2.radioResourceConfigCommon.rachConfigCommon.raSupervisionInfo.raResponseWindowSize =
	...
	case HANDOVER_JOINING: // there may be some delays in the TX of RRC messages, thus an handover may be completed at UE side, but not at eNB side
	{
		...
		NS_LOG_INFO(
				"UeManager is in CONNECTION_RECONFIGURATION, postpone the PrepareHandover command to cell " << m_queuedHandoverRequestCellId);
	}
		break;
 ...
 */
std::vector<LteRlcAm::RetxPdu> UeManager::MergeBuffers(
		std::vector<LteRlcAm::RetxPdu> first,
	...
	while (it_1 != first.end() && it_2 != second.end()) {
		...
		if (!end_1_reached && !end_2_reached) {
			...
			if (rlcamHeader_1.GetSequenceNumber()
					> rlcamHeader_2.GetSequenceNumber()) {
				result.push_back((*it_2));
				++it_2;
			} else if (rlcamHeader_2.GetSequenceNumber()
					> rlcamHeader_1.GetSequenceNumber()) {
			...
			NS_LOG_DEBUG(
					"first,second = " << rlcamHeader_1.GetSequenceNumber() << "," << rlcamHeader_2.GetSequenceNumber());
		} else {
			break;
		}
	}
...
void UeManager::RecvHandoverRequestAck(
	...
	for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator drbIt =
			m_drbMap.begin(); drbIt != m_drbMap.end(); ++drbIt) {
	...
	if (m_rrc->m_ismmWave && m_isMc) {
		...
		for (std::map<uint8_t, Ptr<RlcBearerInfo> >::iterator rlcIt =
	...
	NS_LOG_INFO(
			"m_drbMap size " << m_drbMap.size() << " in cell " << m_rrc->m_cellId << " forward RLC buffers");
	for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator drbIt =
			m_drbMap.begin(); drbIt != m_drbMap.end(); ++drbIt) {
		ForwardRlcBuffers(drbIt->second->m_rlc, drbIt->second->m_pdcp,
				drbIt->second->m_gtpTeid, 0, 0, 0);
	}

	if (m_rrc->m_ismmWave && m_isMc) // for secondary cell HO
			{
		NS_LOG_INFO(
				"m_rlcMap size " << m_rlcMap.size() << " in cell " << m_rrc->m_cellId << " forward RLC buffers");
		for (std::map<uint8_t, Ptr<RlcBearerInfo> >::iterator rlcIt =
				m_rlcMap.begin(); rlcIt != m_rlcMap.end(); ++rlcIt) {
			// the buffers are forwarded to m_targetCellId, which is set in PrepareHandover
			// the target cell
			ForwardRlcBuffers(rlcIt->second->m_rlc, 0, rlcIt->second->gtpTeid,
					0, 1, 0);
		}
	}
}
...
void UeManager::ForwardRlcBuffers(Ptr<LteRlc> rlc, Ptr<LtePdcp> pdcp,
		uint32_t gtpTeid, bool mcLteToMmWaveForwarding,
		bool mcMmToMmWaveForwarding, uint8_t bid) {
	...
	if (0 != rlc->GetObject<LteRlcAm>()) {
		...
		uint32_t pos = 0;
		for (std::vector<Ptr<Packet> >::iterator it = txonBuffer.begin();
				it != txonBuffer.end(); ++it) {
			pos++;
			if ((*it)->GetSize() > 3) {
				(*it)->PeekHeader(pdcpHeader);
				NS_LOG_DEBUG(
						"txonBuffer SEQ = " << pdcpHeader.GetSequenceNumber() << " Size = " << (*it)->GetSize());
			} else {
		...
		if (rlcAm->GetTransmittingRlcSduBufferSize() > 0) { //something inside the RLC AM's transmitting buffer
			NS_LOG_DEBUG(
					"ADDING TRANSMITTING SDUS OF RLC AM TO X2FORWARDINGBUFFER... Size = " << rlcAm->GetTransmittingRlcSduBufferSize());
			...
			for (std::map<uint32_t, Ptr<Packet> >::iterator it =
					rlcAmTransmittingBuffer.begin();
					it != rlcAmTransmittingBuffer.end(); ++it) {
				if (it->second != 0) {
					NS_LOG_DEBUG(
							this << " add to forwarding buffer SEQ = " << it->first << " Ptr<Packet> = " << it->second);
			...
			if (segmentedRlcsdu != NULL) {
				...
				NS_LOG_DEBUG(
						this << "SegmentedRlcSdu = " << segmentedRlcsdu->GetSize() << " SEQ = " << pdcpHeader.GetSequenceNumber());
			...
			uint16_t i = 0;
			for (std::vector<Ptr<Packet> >::iterator it =
					rlcAmTxedSduBuffer.begin(); it != rlcAmTxedSduBuffer.end();
					++it) {
				if ((*it) != NULL) {
					(*it)->PeekHeader(pdcpHeader);
					NS_LOG_DEBUG(
							"rlcAmTxedSduBuffer SEQ = " << pdcpHeader.GetSequenceNumber() << " Size = " << (*it)->GetSize());

					//add the previous SDU of the forwarding buffer to the forwarding buffer.
					if (pdcpHeader.GetSequenceNumber()
							>= (pdcpHeader_1.GetSequenceNumber() - 2)
							&& pdcpHeader.GetSequenceNumber()
									<= (pdcpHeader_1.GetSequenceNumber())) {
						NS_LOG_DEBUG(
								"Added previous SDU to forwarding buffer SEQ = " << pdcpHeader.GetSequenceNumber() << " Size = " << (*it)->GetSize());
						m_x2forwardingBuffer.insert(
								m_x2forwardingBuffer.begin() + i,
								(*it)->Copy());
						++i;
					}
				}
	...
	//be correct).
		...
		m_x2forwardingBufferSize =
				rlc->GetObject<LteRlcUm>()->GetTxBufferSize();
		...
		m_x2forwardingBufferSize =
				rlc->GetObject<LteRlcUmLowLat>()->GetTxBufferSize();
	}
	...
	while (!m_x2forwardingBuffer.empty()) {
		NS_LOG_DEBUG(
				this << " Forwarding m_x2forwardingBuffer to target eNB, gtpTeid = " << gtpTeid);
		...
		if (rlcSdu->GetSize() >= 3) {
			...
			if (pdcpHeader.GetDcBit() == 1) { //ignore control SDU.
				...
				NS_LOG_LOGIC(
						"removed pdcp header, size = " << rlcSdu->GetSize());
					   ...
					   // Thus this method must be called after the switch to MmWave is done!
				{
					...
					mcPdcp->GetLtePdcpSapProvider()->TransmitPdcpSdu(
							pdcpParams);
				}
			}
		} else {
		...
		m_x2forwardingBufferSize -=
				(*(m_x2forwardingBuffer.begin()))->GetSize();
...
void UeManager::SendHoldBufferMsg (EpcX2SapUser::HoldBufferParams params){
	NS_LOG_FUNCTION(this);
...
LteRrcSap::RadioResourceConfigDedicated UeManager::GetRadioResourceConfigForHandoverPreparationInfo() {
	NS_LOG_FUNCTION(this);
	return BuildRadioResourceConfigDedicated();
}

LteRrcSap::RrcConnectionReconfiguration UeManager::GetRrcConnectionReconfigurationForHandover() {
	NS_LOG_FUNCTION(this);
...
void UeManager::SendData(uint8_t bid, Ptr<Packet> p) {
	...
	case HANDOVER_PATH_SWITCH: {
		...
		std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =
				m_drbMap.find(drbid);
		if (it != m_drbMap.end()) {
			Ptr<LteDataRadioBearerInfo> bearerInfo = GetDataRadioBearerInfo(
					drbid);
			if (bearerInfo != NULL) {
...
void UeManager::RecvHandoverPreparationFailure(uint16_t cellId) {
	...
	case HANDOVER_LEAVING:
		NS_ASSERT(cellId == m_targetCellId);
		NS_LOG_INFO(
				"target eNB sent HO preparation failure, aborting HO because RrcConnectionReconfigurationCompleted was not received at target");
		break;
...
void UeManager::RecvSnStatusTransfer(
	...
	for (std::vector<EpcX2Sap::ErabsSubjectToStatusTransferItem>::iterator erabIt =
		...
		NS_LOG_DEBUG(
				"SN STATUS RECEIVED txSn, rxSn = " << status.txSn << "," << status.rxSn);
...
void UeManager::RecvUeContextRelease(
	...
	if (m_rrc->m_ismmWave && m_isMc) {
		for (std::map<uint8_t, Ptr<RlcBearerInfo> >::iterator rlcIt =
				m_rlcMap.begin(); rlcIt != m_rlcMap.end(); ++rlcIt) {
			NS_LOG_INFO(
					"Remove the X2 forward for TEID " << rlcIt->second->gtpTeid);
			m_rrc->m_x2SapProvider->RemoveTeidToBeForwarded(
					rlcIt->second->gtpTeid);
		}
	}
}

void UeManager::RecvRlcSetupRequest(EpcX2SapUser::RlcSetupRequest params) // TODO only on MC
		{
	if (m_isMc) {
		NS_LOG_INFO("Setup remote RLC in cell " << m_rrc->GetCellId());
		NS_ASSERT_MSG(
				m_state == HANDOVER_JOINING || params.mmWaveRnti == m_rnti,
		...
		ret = m_rrc->m_x2uMcTeidInfoMap.insert(
				std::pair<uint32_t, LteEnbRrc::X2uTeidInfo>(params.gtpTeid,
						x2uTeidInfo));
		...
		if (rlcTypeId != LteRlcSm::GetTypeId()) {
			...
			m_rrc->m_x2SapProvider->SetEpcX2RlcUser(params.gtpTeid,
					rlc->GetEpcX2RlcUser());
		}
		...
		rlcInfo->logicalChannelConfig.priority =
				params.logicalChannelConfig.priority;
...
void UeManager::RecvRlcSetupCompleted(uint8_t drbid) {
	NS_ASSERT_MSG(m_drbMap.find(drbid) != m_drbMap.end(),
			"The drbid does not match");
...
void UeManager::RecvRrcConnectionRequest(LteRrcSap::RrcConnectionRequest msg) {
	...
	case INITIAL_RANDOM_ACCESS: {
		...
		if (m_rrc->m_admitRrcConnectionRequest == true) {
			...
			NS_LOG_DEBUG(
					"For imsi " << m_imsi << " m_rrc->m_mmWaveCellSetupCompleted[m_imsi] " << m_rrc->m_mmWaveCellSetupCompleted[m_imsi]);
			...
			msg2.radioResourceConfigDedicated =
...
void UeManager::RecvRrcConnectionSetupCompleted(
	...
	case CONNECTION_SETUP:
		...
		if (m_rrc->m_bestMmWaveCellForImsiMap[m_imsi] != m_rrc->GetCellId()
				&& !m_rrc->m_ismmWave) {
			uint16_t maxSinrCellId = m_rrc->m_bestMmWaveCellForImsiMap[m_imsi];
			// get the SINR
			double maxSinrDb = 10
					* std::log10(
							m_rrc->m_imsiCellSinrMap.find(m_imsi)->second.find(
									maxSinrCellId)->second);
			if (maxSinrDb > m_rrc->m_outageThreshold) {
				...
				NS_LOG_INFO(
						"Send connect to " << m_rrc->m_bestMmWaveCellForImsiMap.find(m_imsi)->second << " at least one mmWave eNB is not in outage");
				m_rrc->m_rrcSapUser->SendRrcConnectToMmWave(m_rnti,
...
bool UeManager::GetAllMmWaveInOutageAtInitialAccess() {
	return m_allMmWaveInOutageAtInitialAccess;
}

void UeManager::SetAllMmWaveInOutageAtInitialAccess(bool param) {
...
void UeManager::RecvRrcConnectionReconfigurationCompleted(
		...
		for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =
			...
			if (it->second->m_isMc) {
				...
				if (pdcp != 0) {
					...
					NS_LOG_INFO(
							"Imsi " << m_imsi << " m_mmWaveCellSetupCompleted set to " << m_rrc->m_mmWaveCellSetupCompleted[m_imsi] << " for cell " << m_rrc->m_lastMmWaveCell[m_imsi]);
					m_rrc->m_imsiUsingLte[m_imsi] = false;
					ForwardRlcBuffers(it->second->m_rlc, pdcp,
							it->second->m_gtpTeid, 1, 0, it->first);
				} else {
					NS_FATAL_ERROR(
							"A device defined as MC has not a McEnbPdcp");
				}
			}
		}
	...
	case CONNECTION_RECONFIGURATION:
		...
		if (m_needPhyMacConfiguration) {
			...
			// configure PHY
			m_rrc->m_cphySapProvider->SetTransmissionMode(req.m_rnti,
					req.m_transmissionMode);
		...
		if (m_mmWaveCellAvailableForMcSetup) {
			NS_LOG_INFO(
					"Notify the secondary cell that some bearers' RLC can be setup");
			NS_ASSERT_MSG((m_mmWaveCellId != 0) && (m_mmWaveRnti != 0),
					"Unkonwn secondary MmWave cell");
			RecvRrcSecondaryCellInitialAccessSuccessful(m_mmWaveRnti,
					m_mmWaveCellId);
		}
		if (m_receivedLteMmWaveHandoverCompleted) {
			NS_LOG_INFO(
					"Notify LteEnbRrc that LTE cell received a NotifyLteMmWaveHandoverCompleted and has completed CONNECTION_RECONFIGURATION");
		...
		if (m_rrc->m_interRatHoMode && m_firstConnection
				&& !m_rrc->m_ismmWave) {
			m_rrc->m_mmWaveCellSetupCompleted[m_imsi] = true;
			m_rrc->m_lastMmWaveCell[m_imsi] = m_rrc->m_cellId;
			m_rrc->m_imsiUsingLte[m_imsi] = true; // the inital connection happened on a LTE eNB
		...
		SwitchToState(CONNECTED_NORMALLY);
		NS_LOG_INFO(
				"m_queuedHandoverRequestCellId " << m_queuedHandoverRequestCellId);
		if (m_queuedHandoverRequestCellId > 0) {
			NS_LOG_INFO(
					"Call the postponed PrepareHandover to cell " << m_queuedHandoverRequestCellId);
			PrepareHandover(m_queuedHandoverRequestCellId);
		}
		m_rrc->m_connectionReconfigurationTrace(m_imsi, m_rrc->m_cellId,
				m_rnti);
		break;
	...
	case HANDOVER_JOINING: {
		...
		if (!m_isMc) {
			...
			NS_LOG_INFO(
					"RRC Reconfiguration completed after secondary cell HO: send path switch to LTE coordinator");
	...
	for (std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator it =
		...
		if (!(it->second->m_isMc)
			...
			if (pdcp != 0) {
				...
				x2uTeidInfo.rnti = m_rnti;
				x2uTeidInfo.drbid = it->first;
				std::pair<std::map<uint32_t, LteEnbRrc::X2uTeidInfo>::iterator,
						bool> ret;
				ret = m_rrc->m_x2uMcTeidInfoMap.insert(
						std::pair<uint32_t, LteEnbRrc::X2uTeidInfo>(
								it->second->m_gtpTeid, x2uTeidInfo));
...
void UeManager::SendRrcConnectionSwitch(bool useMmWaveConnection) {
		...
		if (it->second->m_isMc) {
			...
			if (pdcp != 0) {
				...
				if (!m_rrc->m_ismmWave && useMmWaveConnection) //
					...
					ForwardRlcBuffers(it->second->m_rlc, it->second->m_pdcp,
							it->second->m_gtpTeid, 1, 0, it->first);
					...
					TypeId rlcTypeId = m_rrc->GetRlcType(
							it->second->m_epsBearer);

					ObjectFactory rlcObjectFactory;
					rlcObjectFactory.SetTypeId(rlcTypeId);
					Ptr<LteRlc> rlc = rlcObjectFactory.Create()->GetObject<
							LteRlc>();
					rlc->SetLteMacSapProvider(m_rrc->m_macSapProvider);
					rlc->SetRnti(m_rnti);
					...
					NS_LOG_INFO(
							"Reset RLC in LTE eNB after switch, new rlc " << rlc);
					...
					// if we are using RLC/SM we don't care of anything above RLC
					if (rlcTypeId != LteRlcSm::GetTypeId()) {
						DynamicCast<McEnbPdcp>(it->second->m_pdcp)->SetLteRlcSapProvider(
					...
					lcinfo.rnti = m_rnti;
					lcinfo.lcId = lcid;
					lcinfo.lcGroup = m_rrc->GetLogicalChannelGroup(
							it->second->m_epsBearer);
					...
					m_rrc->m_cmacSapProvider->AddLc(lcinfo,
	...
	NS_LOG_INFO(
			"SendRrcConnectionSwitch to " << m_rnti << " with useMmWaveConnection " << msg.useMmWaveConnection << " at time " << Simulator::Now().GetSeconds());
	m_rrc->m_rrcSapUser->SendRrcConnectionSwitch(m_rnti, msg);
}

void UeManager::RecvConnectionSwitchToMmWave(bool useMmWaveConnection,
		uint8_t drbid) {
	NS_ASSERT_MSG(m_rlcMap.find(drbid) != m_rlcMap.end(),
			"drbid not found in m_rlcMap");
	NS_LOG_INFO(
			"RecvConnectionSwitchToMmWave on cell " << m_rrc->m_cellId << " switch " << useMmWaveConnection << " for drbid " << (uint32_t)drbid);
	if (!useMmWaveConnection) {
		...
		if (rlcTypeId != LteRlcSm::GetTypeId()) {
			...
			rlc->SetUeDataParams(ueParams);
			m_rrc->m_x2SapProvider->SetEpcX2RlcUser(
...
uint16_t UeManager::GetRnti(void) const {
	return m_rnti;
}

uint64_t UeManager::GetImsi(void) const {
	return m_imsi;
}

uint16_t UeManager::GetSrsConfigurationIndex(void) const {
...
UeManager::State UeManager::GetState(void) const {
	return m_state;
}
	...
	for (int drbid = (m_lastAllocatedDrbid + 1) % MAX_DRB_ID;
		...
		if (drbid != 0) // 0 is not allowed
			...
			if (m_drbMap.find(drbid) == m_drbMap.end()) {
				m_drbMap.insert(
						std::pair<uint8_t, Ptr<LteDataRadioBearerInfo> >(drbid,
								drbInfo));
				drbInfo->m_drbIdentity = drbid;
...
LteRrcSap::RrcConnectionReconfiguration UeManager::BuildRrcConnectionReconfiguration() {
	LteRrcSap::RrcConnectionReconfiguration msg;
	msg.rrcTransactionIdentifier = GetNewRrcTransactionIdentifier();
	msg.haveRadioResourceConfigDedicated = true;
...
LteRrcSap::RadioResourceConfigDedicated UeManager::BuildRadioResourceConfigDedicated() {
	LteRrcSap::RadioResourceConfigDedicated rrcd;
	...
	if (m_drbMap.size() == 0 && m_isMc && m_rrc->m_ismmWave) // UeManager on a secondary cell for a MC device
			{
		for (std::map<uint8_t, Ptr<RlcBearerInfo> >::iterator it =
				m_rlcMap.begin(); it != m_rlcMap.end(); ++it) {
			LteRrcSap::DrbToAddMod dtam;
...
uint8_t UeManager::Drbid2Lcid(uint8_t drbid) {
	return drbid + 2;
}
...
uint8_t UeManager::Bid2Lcid(uint8_t bid) {
	return bid + 2;
}

uint8_t UeManager::Drbid2Bid(uint8_t drbid) {
	return drbid;
}

uint8_t UeManager::Bid2Drbid(uint8_t bid) {
	return bid;
}
...
void UeManager::SwitchToState(State newState) {
	...
	m_state = newState;
	NS_LOG_INFO(
			this << " IMSI " << m_imsi << " RNTI " << m_rnti << " CellId " << m_rrc->m_cellId << " UeManager " << ToString (oldState) << " --> " << ToString (newState));
...
void UeManager::SetFirstConnection() {
	m_firstConnection = true;
}

void UeManager::RecvNotifyLteMmWaveHandoverCompleted() {
	...
	case CONNECTED_NORMALLY:
		m_rrc->m_mmWaveCellSetupCompleted.find(m_imsi)->second = true;
		NS_LOG_DEBUG(
				"RecvNotifyLteMmWaveHandoverCompleted imsi " << m_imsi << " m_rrc->m_mmWaveCellSetupCompleted[m_imsi] " << m_rrc->m_mmWaveCellSetupCompleted[m_imsi]);
		m_rrc->m_imsiUsingLte.find(m_imsi)->second = true;
		break;
	default:
...
LteEnbRrc::LteEnbRrc() :
		m_x2SapProvider(0), m_cmacSapProvider(0), m_handoverManagementSapProvider(
				0), m_anrSapProvider(0), m_ffrRrcSapProvider(0), m_rrcSapUser(
				0), m_macSapProvider(0), m_s1SapProvider(0), m_cphySapProvider(
				0), m_configured(false), m_lastAllocatedRnti(0), m_srsCurrentPeriodicityId(
				0), m_lastAllocatedConfigurationIndex(0), m_reconfigureUes(
				false), m_firstSibTime(16), m_numNewSinrReports(0) {
	NS_LOG_FUNCTION(this);
	m_cmacSapUser = new EnbRrcMemberLteEnbCmacSapUser(this);
	m_handoverManagementSapUser = new MemberLteHandoverManagementSapUser<
			LteEnbRrc>(this);
...
TypeId LteEnbRrc::GetTypeId(void) {
	...
	static TypeId tid =
			TypeId("ns3::LteEnbRrc").SetParent<Object>().SetGroupName("Lte").AddConstructor<
					LteEnbRrc>().AddAttribute("UeMap",
					"List of UeManager by C-RNTI.", ObjectMapValue(),
					MakeObjectMapAccessor(&LteEnbRrc::m_ueMap),
					MakeObjectMapChecker<UeManager>()).AddAttribute(
					...
					MakeEnumChecker(RLC_SM_ALWAYS, "RlcSmAlways", RLC_UM_ALWAYS,
							"RlcUmAlways", RLC_AM_ALWAYS, "RlcAmAlways",
							PER_BASED, "PacketErrorRateBased",
							RLC_UM_LOWLAT_ALWAYS, "MmwRlcUmAlways")).AddAttribute(
					"SystemInformationPeriodicity",
					"The interval for sending system information (Time value)",
					TimeValue(MilliSeconds(5)),
					MakeTimeAccessor(
							&LteEnbRrc::m_systemInformationPeriodicity),
					MakeTimeChecker())

			// SRS related attributes
			.AddAttribute("SrsPeriodicity",
					"The SRS periodicity in milliseconds", UintegerValue(40),
			...
			.AddAttribute("ConnectionRequestTimeoutDuration",
					...
					MakeTimeAccessor(
							&LteEnbRrc::m_connectionRequestTimeoutDuration),
					...
					"After accepting connection request, if no RRC CONNECTION "
							...
							"SETUP COMPLETE.", TimeValue(MilliSeconds(150)),
					MakeTimeAccessor(
							&LteEnbRrc::m_connectionSetupTimeoutDuration),
					MakeTimeChecker()).AddAttribute(
					...
					MakeTimeAccessor(
							&LteEnbRrc::m_connectionRejectedTimeoutDuration),
					MakeTimeChecker()).AddAttribute(
					...
					MakeTimeAccessor(
							&LteEnbRrc::m_handoverJoiningTimeoutDuration),
					MakeTimeChecker()).AddAttribute(
			...
			.AddAttribute("AdmitHandoverRequest",
					...
					BooleanValue(true),
					MakeBooleanAccessor(
							&LteEnbRrc::m_admitRrcConnectionRequest),
					MakeBooleanChecker())
			...
			.AddAttribute("RsrpFilterCoefficient",
					...
					MakeUintegerChecker<uint8_t>(0)).AddAttribute(
					...
					MakeBooleanChecker()).AddAttribute("FirstSibTime",
					"Time in ms of initial SIB message",
					// i.e. the variable k in 3GPP TS 36.331 section 5.5.3.2
					UintegerValue(2),
					MakeUintegerAccessor(&LteEnbRrc::m_firstSibTime),
					MakeUintegerChecker<uint32_t>(0)).AddAttribute(
					"InterRatHoMode",
					...
					MakeBooleanChecker()).AddAttribute("HoSinrDifference",
					...
					"SecondaryCellHandoverMode",
					"Select the secondary cell handover mode",
					EnumValue(DYNAMIC_TTT),
					MakeEnumAccessor(&LteEnbRrc::m_handoverMode),
					MakeEnumChecker(FIXED_TTT, "FixedTtt", DYNAMIC_TTT,
							"DynamicTtt", THRESHOLD, "Threshold")).AddAttribute(
					"FixedTttValue",
			...
			.AddAttribute("MaxDiffValue",
					...
					MakeDoubleChecker<double>()) // TODO set the proper value
			.AddAttribute("CrtPeriod", "The periodicity of a CRT (us)",
					IntegerValue(1600),
			...
			.AddTraceSource("NewUeContext",
					...
					"ns3::LteEnbRrc::NewUeContextTracedCallback").AddTraceSource(
					"ConnectionEstablished",
					"Fired upon successful RRC connection establishment.",
					MakeTraceSourceAccessor(
							&LteEnbRrc::m_connectionEstablishedTrace),
					"ns3::LteEnbRrc::ConnectionHandoverTracedCallback").AddTraceSource(
					...
					"ns3::LteEnbRrc::HandoverStartTracedCallback").AddTraceSource(
					"HandoverEndOk",
					...
					MakeTraceSourceAccessor(
							&LteEnbRrc::m_recvMeasurementReportTrace),
					"ns3::LteEnbRrc::ReceiveReportTracedCallback").AddTraceSource(
					"NotifyMmWaveSinr",
...
void LteEnbRrc::SetEpcX2PdcpProvider(EpcX2PdcpProvider * s) {
	NS_LOG_FUNCTION(this << s);
	m_x2PdcpProvider = s;
}

void LteEnbRrc::SetEpcX2RlcProvider(EpcX2RlcProvider * s) {
	NS_LOG_FUNCTION(this << s);
	m_x2RlcProvider = s;
}
...
Ptr<UeManager> LteEnbRrc::GetUeManager(uint16_t rnti) {
	...
	NS_ASSERT_MSG(it != m_ueMap.end(),
			"RNTI " << rnti << " not found in eNB with cellId " << m_cellId);
	return it->second;
}

void LteEnbRrc::RegisterImsiToRnti(uint64_t imsi, uint16_t rnti) {
	if (m_imsiRntiMap.find(imsi) == m_imsiRntiMap.end()) {
		m_imsiRntiMap.insert(std::pair<uint64_t, uint16_t>(imsi, rnti));
		NS_LOG_INFO(
				"New entry in m_imsiRntiMap, for rnti " << rnti << " m_interRatHoMode " << m_interRatHoMode << " m_ismmWave " << m_ismmWave);
...
uint16_t LteEnbRrc::GetRntiFromImsi(uint64_t imsi) {
	if (m_imsiRntiMap.find(imsi) != m_imsiRntiMap.end()) {
...
uint64_t LteEnbRrc::GetImsiFromRnti(uint16_t rnti) {
	if (m_rntiImsiMap.find(rnti) != m_rntiImsiMap.end()) {
...
uint8_t LteEnbRrc::AddUeMeasReportConfig(LteRrcSap::ReportConfigEutra config) {
	NS_LOG_FUNCTION(this);
...
void LteEnbRrc::ConfigureCell(uint8_t ulBandwidth, uint8_t dlBandwidth,
		uint16_t ulEarfcn, uint16_t dlEarfcn, uint16_t cellId) {
	NS_LOG_FUNCTION(
			this << (uint16_t) ulBandwidth << (uint16_t) dlBandwidth << ulEarfcn << dlEarfcn << cellId);
	...
	m_ueMeasConfig.quantityConfig.filterCoefficientRSRP =
			m_rsrpFilterCoefficient;
	m_ueMeasConfig.quantityConfig.filterCoefficientRSRQ =
	...
	Simulator::Schedule(MilliSeconds(m_firstSibTime),
...
void LteEnbRrc::SetInterRatHoMode(void) {
...
uint16_t LteEnbRrc::GetCellId() const {
	return m_cellId;
}

void LteEnbRrc::DoUpdateUeSinrEstimate(
		LteEnbCphySapUser::UeAssociatedSinrInfo info) {
	NS_LOG_FUNCTION(this);

	m_ueImsiSinrMap.clear();
	m_ueImsiSinrMap.insert(info.ueImsiSinrMap.begin(),
			info.ueImsiSinrMap.end());
...
void LteEnbRrc::DoRecvUeSinrUpdate(EpcX2SapUser::UeImsiSinrParams params) {
	NS_LOG_FUNCTION(this);
	NS_LOG_LOGIC("Recv Ue SINR Update from cell " << params.sourceCellId);
	uint16_t mmWaveCellId = params.sourceCellId;
	if (m_cellSinrMap.find(mmWaveCellId) != m_cellSinrMap.end()) { // update the entry
		m_cellSinrMap[mmWaveCellId] = params.ueImsiSinrMap;
		m_numNewSinrReports++;
	} else  // add the entry
	{
		m_cellSinrMap.insert(
				std::pair<uint16_t, ImsiSinrMap>(mmWaveCellId,
						params.ueImsiSinrMap));
		m_numNewSinrReports++;
	}
	// cycle on all the Imsi whose SINR is known in cell mmWaveCellId
	for (std::map<uint64_t, double>::iterator imsiIter =
			params.ueImsiSinrMap.begin();
		...
		if (m_imsiCellSinrMap.find(imsi) != m_imsiCellSinrMap.end()) {
			if (m_imsiCellSinrMap[imsi].find(mmWaveCellId)
					!= m_imsiCellSinrMap[imsi].end()) {
			...
			} else // new cell for this Imsi
			{
				...
				m_imsiCellSinrMap[imsi].insert(
						std::pair<uint16_t, double>(mmWaveCellId, sinr));
			}
		} else // new imsi
		{
			CellSinrMap map;
			map.insert(std::pair<uint16_t, double>(mmWaveCellId, sinr));
			m_imsiCellSinrMap.insert(
					std::pair<uint64_t, CellSinrMap>(imsi, map));
		}
	}

	for (std::map<uint64_t, CellSinrMap>::iterator imsiIter =
			m_imsiCellSinrMap.begin(); imsiIter != m_imsiCellSinrMap.end();
			++imsiIter) {
		...
		for (CellSinrMap::iterator cellIter = imsiIter->second.begin();
			...
			NS_LOG_LOGIC(
					"mmWaveCell " << cellIter->first << " sinr " << cellIter->second);
		}
	}
	...
	if (!m_ismmWave && !m_interRatHoMode && m_firstReport) {
		...
		Simulator::Schedule(MilliSeconds(0),
				&LteEnbRrc::TriggerUeAssociationUpdate, this);
	} else if (!m_ismmWave && m_interRatHoMode && m_firstReport) {
		m_firstReport = false;
		Simulator::Schedule(MilliSeconds(0),
				&LteEnbRrc::UpdateUeHandoverAssociation, this);
	}

}

void LteEnbRrc::TttBasedHandover(
		std::map<uint64_t, CellSinrMap>::iterator imsiIter,
	...
	if (m_mmWaveCellSetupCompleted.find(imsi)
			!= m_mmWaveCellSetupCompleted.end()) {
	...
	NS_LOG_INFO(
			"TttBasedHandover: alreadyAssociatedImsi " << alreadyAssociatedImsi << " onHandoverImsi " << onHandoverImsi);
	...
	if (alreadyAssociatedImsi && m_imsiUsingLte[imsi]) {
		if (!m_interRatHoMode) {
			if (GetUeManager(GetRntiFromImsi(imsi))->GetAllMmWaveInOutageAtInitialAccess()) {
				NS_LOG_INFO(
						"Send connect to " << maxSinrCellId << ", for the first time at least one mmWave eNB is not in outage");
				m_rrcSapUser->SendRrcConnectToMmWave(GetRntiFromImsi(imsi),
						maxSinrCellId);
				GetUeManager(GetRntiFromImsi(imsi))->SetAllMmWaveInOutageAtInitialAccess(
						false);
			} else if (m_lastMmWaveCell[imsi] == maxSinrCellId
					&& !onHandoverImsi)
					// it is on LTE, but now the last used MmWave cell is not in outage
					{
				// switch back to MmWave
				NS_LOG_INFO(
						"----- on LTE, switch to lastMmWaveCell " << m_lastMmWaveCell[imsi] << " at time " << Simulator::Now().GetSeconds());
				...
				NS_LOG_INFO(
						"----- on LTE, switch to new MmWaveCell " << maxSinrCellId << " at time " << Simulator::Now().GetSeconds());
				// trigger ho via X2
			...
			if (!onHandoverImsi) {
				...
				NS_LOG_INFO(
						"---- on LTE, handover to MmWave " << maxSinrCellId << " at time " << Simulator::Now().GetSeconds());
		...
		HandoverEventMap::iterator handoverEvent = m_imsiHandoverEventsMap.find(
				imsi);
		if (handoverEvent != m_imsiHandoverEventsMap.end()) {
			...
			if (handoverEvent->second.targetCellId == maxSinrCellId) {
				if (currentSinrDb < m_outageThreshold) // we need to handover right now!
					...
					handoverNeeded = true;
					NS_LOG_INFO(
							"------ Handover was already scheduled, but the current cell is in outage, thus HO to " << maxSinrCellId);
				} else {
					...
					NS_LOG_INFO(
							"Scheduled for " << handoverHappensAtTime << " while now the scheduler would give " << Simulator::Now().GetMilliSeconds() + newTtt);
					if (Simulator::Now().GetMilliSeconds() + newTtt
							< (double) handoverHappensAtTime / 1e6) {
						handoverEvent->second.scheduledHandoverEvent.Cancel();
						NS_LOG_INFO(
								"------ Handover remains scheduled for " << maxSinrCellId << " but a new shorter TTT is computed");
						handoverNeeded = true;
					}
				}
			} else {
				...
				NS_LOG_INFO(
						"------ Handover was scheduled for " << handoverEvent->second.targetCellId << " but now maxSinrCellId is " << maxSinrCellId);
				//  get the SINR for the scheduled targetCellId: if the diff is smaller than 3 dB handover anyway
				double originalTargetSinrDb =
						10
								* std::log10(
										m_imsiCellSinrMap.find(imsi)->second[targetCellId]);
				if (maxSinrDb - originalTargetSinrDb
					...
					if (maxSinrCellId == m_lastMmWaveCell[imsi]) {
						// delete this event
						NS_LOG_INFO(
								"-------------- The difference between the two mmWave SINR is smaller than " << m_sinrThresholdDifference << " dB, but the new max is the current cell, thus cancel the handover");
						handoverEvent->second.scheduledHandoverEvent.Cancel();
						m_imsiHandoverEventsMap.erase(handoverEvent);
					} else {
						NS_LOG_INFO(
								"-------------- The difference between the two mmWave SINR is smaller than " << m_sinrThresholdDifference << " dB, do not cancel the handover");
					}
				}
			}
		} else {
			...
			if (maxSinrCellId != m_lastMmWaveCell[imsi]) {
				NS_LOG_INFO(
						"----- Handover needed from cell " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId);
				handoverNeeded = true;
			}
		}
	}
	...
	if (handoverNeeded) {
		...
		NS_LOG_INFO(
				"The sinrDifference is " << sinrDifference << " and the TTT computed is " << (uint32_t)millisecondsToHandover << " ms, thus the event will happen at time " << Simulator::Now().GetMilliSeconds() + millisecondsToHandover);
		...
		EventId scheduledHandoverEvent = Simulator::Schedule(
				MilliSeconds(millisecondsToHandover),
				&LteEnbRrc::PerformHandover, this, imsi);
...
uint8_t LteEnbRrc::ComputeTtt(double sinrDifference) {
	if (m_handoverMode == FIXED_TTT) {
		...
		{
			double ttt = m_maxDynTttValue
					- (m_maxDynTttValue - m_minDynTttValue)
...
void LteEnbRrc::PerformHandover(uint64_t imsi) {
	NS_ASSERT_MSG(
			m_imsiHandoverEventsMap.find(imsi) != m_imsiHandoverEventsMap.end(),
			"No handover event for this imsi!");
	LteEnbRrc::HandoverEventInfo handoverInfo = m_imsiHandoverEventsMap.find(
			imsi)->second;
	NS_ASSERT_MSG(handoverInfo.sourceCellId == m_lastMmWaveCell[imsi],
			"The secondary cell to which the UE is attached has changed handoverInfo.sourceCellId " << handoverInfo.sourceCellId << " m_lastMmWaveCell[imsi] " << m_lastMmWaveCell[imsi] << " imsi " << imsi);
	bool alreadyAssociatedImsi = false;
	bool onHandoverImsi = true;
	// On RecvRrcConnectionRequest for a new RNTI, the Lte Enb RRC stores the imsi
	// of the UE and insert a new false entry in m_mmWaveCellSetupCompleted.
	...
	NS_LOG_INFO(
			"PerformHandover: alreadyAssociatedImsi " << alreadyAssociatedImsi << " onHandoverImsi " << onHandoverImsi);

	if (alreadyAssociatedImsi) {
		if (!onHandoverImsi) {
			// The new secondary cell HO procedure does not require to switch to LTE
			NS_LOG_INFO(
					"PerformHandover ----- handover from " << m_lastMmWaveCell[imsi] << " to " << handoverInfo.targetCellId << " at time " << Simulator::Now().GetSeconds());
...
void LteEnbRrc::ThresholdBasedSecondaryCellHandover(
		std::map<uint64_t, CellSinrMap>::iterator imsiIter,
		double sinrDifference, uint16_t maxSinrCellId, double maxSinrDb) {
	uint64_t imsi = imsiIter->first;
	bool alreadyAssociatedImsi = false;
	...
	NS_LOG_INFO(
			"ThresholdBasedSecondaryCellHandover: alreadyAssociatedImsi " << alreadyAssociatedImsi << " onHandoverImsi " << onHandoverImsi);

	if (maxSinrCellId == m_bestMmWaveCellForImsiMap[imsi]
			&& !m_imsiUsingLte[imsi]) {
		if (alreadyAssociatedImsi && !onHandoverImsi
				...
				&& sinrDifference > m_sinrThresholdDifference) // not on LTE, handover between MmWave cells
						...
						{
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId << " channel changed previously at time " << Simulator::Now().GetSeconds());
			...
			m_bestMmWaveCellForImsiMap[imsi] = maxSinrCellId;
			NS_LOG_INFO(
					"For imsi " << imsi << " the best cell is " << m_bestMmWaveCellForImsiMap[imsi] << " with SINR " << maxSinrDb);
		} else if (alreadyAssociatedImsi && !onHandoverImsi
				&& m_lastMmWaveCell[imsi] != maxSinrCellId
				&& sinrDifference < m_sinrThresholdDifference) {
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId << " not triggered due to small diff " << sinrDifference << " at time " << Simulator::Now().GetSeconds());
		}
	} else {
		if (alreadyAssociatedImsi && !onHandoverImsi && m_imsiUsingLte[imsi]
			...
			// The new secondary cell HO procedure does not require to switch to LTE
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId << " at time " << Simulator::Now().GetSeconds());
			//Ptr<UeManager> ueMan = GetUeManager(GetRntiFromImsi(imsi));
			//bool useMmWaveConnection = false;
			//m_imsiUsingLte[imsi] = !useMmWaveConnection;
			//ueMan->SendRrcConnectionSwitch(useMmWaveConnection);
...
void LteEnbRrc::TriggerUeAssociationUpdate() {
	if (m_imsiCellSinrMap.size() > 0) // there are some entries
		...
		for (std::map<uint64_t, CellSinrMap>::iterator imsiIter =
			...
			NS_LOG_INFO(
					"alreadyAssociatedImsi " << alreadyAssociatedImsi << " onHandoverImsi " << onHandoverImsi);

			for (CellSinrMap::iterator cellIter = imsiIter->second.begin();
					cellIter != imsiIter->second.end(); ++cellIter) {
				NS_LOG_INFO(
						"Cell " << cellIter->first << " reports " << 10*std::log10(cellIter->second));
			...
			if ((maxSinrDb < m_outageThreshold
				...
				if (m_imsiUsingLte[imsi] == false) {
					...
					HandoverEventMap::iterator handoverEvent =
							m_imsiHandoverEventsMap.find(imsi);
					if (handoverEvent != m_imsiHandoverEventsMap.end()) {
						handoverEvent->second.scheduledHandoverEvent.Cancel();
						m_imsiHandoverEventsMap.erase(handoverEvent);
					}
	...
	Simulator::Schedule(MicroSeconds(m_crtPeriod),
			&LteEnbRrc::TriggerUeAssociationUpdate, this);
}

void LteEnbRrc::ThresholdBasedInterRatHandover(
		std::map<uint64_t, CellSinrMap>::iterator imsiIter,
		double sinrDifference, uint16_t maxSinrCellId, double maxSinrDb) {
	uint64_t imsi = imsiIter->first;
	bool alreadyAssociatedImsi = false;
	...
	if (maxSinrCellId == m_bestMmWaveCellForImsiMap[imsi]
		...
		if (alreadyAssociatedImsi && !onHandoverImsi
				...
				&& sinrDifference > m_sinrThresholdDifference) // not on LTE, handover between MmWave cells
						...
						{
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId << " channel changed previously");
			// trigger ho via X2
			EpcX2SapProvider::SecondaryHandoverParams params;
			params.imsi = imsi;
			params.targetCellId = maxSinrCellId;
			...
			NS_LOG_INFO(
				...
				&& sinrDifference < m_sinrThresholdDifference) {
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId << " not triggered due to small diff " << sinrDifference);
		}
	} else {
		...
		// it is on LTE, but now the a MmWave cell is not in outage
			...
			m_bestMmWaveCellForImsiMap[imsi] = maxSinrCellId;
			NS_LOG_INFO(
					"Handover to MmWave " << m_bestMmWaveCellForImsiMap[imsi]);
			SendHandoverRequest(GetRntiFromImsi(imsi),
					m_bestMmWaveCellForImsiMap[imsi]);
		} else if (!m_imsiUsingLte[imsi] && alreadyAssociatedImsi
				&& !onHandoverImsi && m_lastMmWaveCell[imsi] != maxSinrCellId
				&& sinrDifference > m_sinrThresholdDifference) // not on LTE, handover between MmWave cells
						{
			NS_LOG_INFO(
					"----- handover from " << m_lastMmWaveCell[imsi] << " to " << maxSinrCellId);
			// trigger ho via X2
			EpcX2SapProvider::SecondaryHandoverParams params;
			params.imsi = imsi;
			params.targetCellId = maxSinrCellId;
...
void LteEnbRrc::EnableSwitching(uint64_t imsi) {
	m_switchEnabled = true;
}

void LteEnbRrc::UpdateUeHandoverAssociation() {
	...
	if (m_imsiCellSinrMap.size() > 0) // there are some entries
		...
		for (std::map<uint64_t, CellSinrMap>::iterator imsiIter =
			...
			{
				if (m_handoverMode == THRESHOLD) {
					ThresholdBasedInterRatHandover(imsiIter, sinrDifference,
...
void LteEnbRrc::DoRecvMcHandoverRequest(
...
void LteEnbRrc::SendHandoverRequest(uint16_t rnti, uint16_t cellId) {
	...
	NS_LOG_INFO(
			"LteEnbRrc on cell " << m_cellId << " for rnti " << rnti << " SendHandoverRequest at time " << Simulator::Now().GetSeconds() << " to cellId " << cellId);
...
void LteEnbRrc::DoCompleteSetupUe(uint16_t rnti,
		LteEnbRrcSapProvider::CompleteSetupUeParameters params) {
	NS_LOG_FUNCTION(this << rnti);
	GetUeManager(rnti)->CompleteSetupUe(params);
}

void LteEnbRrc::DoRecvRrcConnectionRequest(uint16_t rnti,
		LteRrcSap::RrcConnectionRequest msg) {
	NS_LOG_FUNCTION(this << rnti);
	GetUeManager(rnti)->RecvRrcConnectionRequest(msg);
}

void LteEnbRrc::DoRecvRrcConnectionSetupCompleted(uint16_t rnti,
		LteRrcSap::RrcConnectionSetupCompleted msg) {
	NS_LOG_FUNCTION(this << rnti);
	GetUeManager(rnti)->RecvRrcConnectionSetupCompleted(msg);
}

void LteEnbRrc::DoRecvRrcConnectionReconfigurationCompleted(uint16_t rnti,
		LteRrcSap::RrcConnectionReconfigurationCompleted msg) {
	NS_LOG_FUNCTION(this << rnti);
	NS_LOG_INFO(
			"Received RRC connection reconf completed on cell " << m_cellId);
	GetUeManager(rnti)->RecvRrcConnectionReconfigurationCompleted(msg);
}

void LteEnbRrc::DoRecvRrcConnectionReestablishmentRequest(uint16_t rnti,
		LteRrcSap::RrcConnectionReestablishmentRequest msg) {
	NS_LOG_FUNCTION(this << rnti);
	GetUeManager(rnti)->RecvRrcConnectionReestablishmentRequest(msg);
}

void LteEnbRrc::DoRecvRrcConnectionReestablishmentComplete(uint16_t rnti,
		LteRrcSap::RrcConnectionReestablishmentComplete msg) {
	NS_LOG_FUNCTION(this << rnti);
...
void LteEnbRrc::DoRecvRrcSecondaryCellInitialAccessSuccessful(uint16_t rnti,
		uint16_t mmWaveRnti, uint16_t mmWaveCellId) {
	NS_LOG_FUNCTION(this << rnti);
	GetUeManager(rnti)->RecvRrcSecondaryCellInitialAccessSuccessful(mmWaveRnti,
			mmWaveCellId);
}
...
void LteEnbRrc::DoDataRadioBearerSetupRequest(
	...
	ueManager->SetupDataRadioBearer(request.bearer, request.bearerId,
			request.gtpTeid, request.transportLayerAddress);
}
...
void LteEnbRrc::DoRecvHandoverRequest(EpcX2SapUser::HandoverRequestParams req) {
	...
	LteEnbCmacSapProvider::AllocateNcRaPreambleReturnValue anrcrv =
			m_cmacSapProvider->AllocateNcRaPreamble(rnti);
	...
	for (std::vector<EpcX2Sap::RlcSetupRequest>::iterator it =
			req.rlcRequests.begin(); it != req.rlcRequests.end(); ++it) {
	...
	handoverCommand.mobilityControlInfo.carrierBandwidth.dlBandwidth =
			m_dlBandwidth;
	handoverCommand.mobilityControlInfo.carrierBandwidth.ulBandwidth =
			m_ulBandwidth;
	...
	handoverCommand.mobilityControlInfo.radioResourceConfigCommon.rachConfigCommon.preambleInfo.numberOfRaPreambles =
			rc.numberOfRaPreambles;
	handoverCommand.mobilityControlInfo.radioResourceConfigCommon.rachConfigCommon.raSupervisionInfo.preambleTransMax =
			rc.preambleTransMax;
	handoverCommand.mobilityControlInfo.radioResourceConfigCommon.rachConfigCommon.raSupervisionInfo.raResponseWindowSize =
			rc.raResponseWindowSize;
	...
	//Process1 Hold Buffer message to RLC

	EpcX2SapProvider::HoldBufferParams hbParams;
	hbParams.sourceCellId = req.sourceCellId;
...
void LteEnbRrc::DoRecvLteMmWaveHandoverCompleted(
	...
	NS_LOG_INFO(
			"Recv X2 message: NOTIFY MMWAVE HANDOVER COMPLETED " << m_interRatHoMode << " m_ismmWave " << m_ismmWave);

	if (m_interRatHoMode && !m_ismmWave) {
		uint64_t imsi = params.imsi;
		NS_LOG_INFO(
				"LTE eNB received notification that MmWave handover is completed to cell " << params.targetCellId);
...
void LteEnbRrc::DoRecvConnectionSwitchToMmWave(
		EpcX2SapUser::SwitchConnectionParams params) {
	NS_LOG_FUNCTION(this);

	NS_LOG_LOGIC("Recv X2 message: SWITCH CONNECTION");

	GetUeManager(params.mmWaveRnti)->RecvConnectionSwitchToMmWave(
			params.useMmWaveConnection, params.drbid);
}
...
void LteEnbRrc::DoRecvRlcSetupRequest(EpcX2SapUser::RlcSetupRequest params) {
	NS_LOG_FUNCTION(this);
...
void LteEnbRrc::DoRecvRlcSetupCompleted(EpcX2SapUser::UeDataParams params) {
	NS_LOG_FUNCTION(this);

	NS_LOG_LOGIC("Recv X2 message: RLC SETUP COMPLETED");

	std::map<uint32_t, X2uTeidInfo>::iterator teidInfoIt =
			m_x2uMcTeidInfoMap.find(params.gtpTeid);
	if (teidInfoIt != m_x2uMcTeidInfoMap.end()) {
		GetUeManager(teidInfoIt->second.rnti)->RecvRlcSetupCompleted(
				teidInfoIt->second.drbid);
	} else {
...
void LteEnbRrc::DoRecvUeData(EpcX2SapUser::UeDataParams params) {
	...
	if (teidInfoIt != m_x2uTeidInfoMap.end()) {
		GetUeManager(teidInfoIt->second.rnti)->SendData(
				teidInfoIt->second.drbid, params.ueData);
	} else {
...
uint8_t LteEnbRrc::DoAddUeMeasReportConfigForHandover(
...
void LteEnbRrc::DoTriggerHandover(uint16_t rnti, uint16_t targetCellId) {
	...
	if (m_anrSapProvider != 0) {
		...
		NS_LOG_DEBUG(
				this << " cellId=" << m_cellId << " targetCellId=" << targetCellId << " NRT.NoHo=" << noHo << " NRT.NoX2=" << noX2);

		if (noHo || noX2) {
			isHandoverAllowed = false;
			NS_LOG_LOGIC(
					this << " handover to cell " << targetCellId << " is not allowed by ANR");
		}
	}
	...
	if (ueManager->GetState() != UeManager::CONNECTED_NORMALLY) {
		isHandoverAllowed = false;
		NS_LOG_LOGIC(
				this << " handover is not allowed because the UE" << " rnti=" << rnti << " is in " << ToString (ueManager->GetState ()) << " state");
	}
...
void LteEnbRrc::DoSetPdschConfigDedicated(uint16_t rnti,
		LteRrcSap::PdschConfigDedicated pdschConfigDedicated) {
	NS_LOG_FUNCTION(this);
	Ptr<UeManager> ueManager = GetUeManager(rnti);
...
uint16_t LteEnbRrc::AddUe(UeManager::State state) {
	NS_LOG_FUNCTION(this);
	bool found = false;
	uint16_t rnti;
 ...
 */
static const uint16_t g_srsPeriodicity[SRS_ENTRIES] = { 0, 2, 5, 10, 20, 40, 80,
		160, 320 };
/**
 ...
 */
static const uint16_t g_srsCiLow[SRS_ENTRIES] = { 0, 0, 2, 7, 17, 37, 77, 157,
		317 };
/**
 ...
 * Table 8.2-1. Index starts from 1.
 */
static const uint16_t g_srsCiHigh[SRS_ENTRIES] = { 0, 1, 6, 16, 36, 76, 156,
		316, 636 };
...
void LteEnbRrc::SetSrsPeriodicity(uint32_t p) {
	...
	NS_FATAL_ERROR(
			"illecit SRS periodicity value " << p << ". Allowed values: " << allowedValues.str ());
}
...
uint16_t LteEnbRrc::GetNewSrsConfigurationIndex() {
	...
	NS_ASSERT(m_srsCurrentPeriodicityId < SRS_ENTRIES);
	NS_LOG_DEBUG(
			this << " SRS p " << g_srsPeriodicity[m_srsCurrentPeriodicityId] << " set " << m_ueSrsConfigurationIndexSet.size ());
	if (m_ueSrsConfigurationIndexSet.size()
			>= g_srsPeriodicity[m_srsCurrentPeriodicityId]) {
		NS_FATAL_ERROR(
				"too many UEs (" << m_ueSrsConfigurationIndexSet.size () + 1 << ") for current SRS periodicity " << g_srsPeriodicity[m_srsCurrentPeriodicityId] << ", consider increasing the value of ns3::LteEnbRrc::SrsPeriodicity");
	}

	if (m_ueSrsConfigurationIndexSet.empty()) {
		// first entry
		m_lastAllocatedConfigurationIndex =
				g_srsCiLow[m_srsCurrentPeriodicityId];
		m_ueSrsConfigurationIndexSet.insert(m_lastAllocatedConfigurationIndex);
	} else {
		// find a CI from the available ones
		std::set<uint16_t>::reverse_iterator rit =
				m_ueSrsConfigurationIndexSet.rbegin();
		NS_ASSERT(rit != m_ueSrsConfigurationIndexSet.rend());
		NS_LOG_DEBUG(
				this << " lower bound " << (*rit) << " of " << g_srsCiHigh[m_srsCurrentPeriodicityId]);
		if ((*rit) < g_srsCiHigh[m_srsCurrentPeriodicityId]) {
			// got it from the upper bound
			m_lastAllocatedConfigurationIndex = (*rit) + 1;
			m_ueSrsConfigurationIndexSet.insert(
					m_lastAllocatedConfigurationIndex);
		} else {
			...
			for (uint16_t srcCi = g_srsCiLow[m_srsCurrentPeriodicityId];
				...
				std::set<uint16_t>::iterator it =
						m_ueSrsConfigurationIndexSet.find(srcCi);
...
uint8_t LteEnbRrc::GetLogicalChannelGroup(bool isGbr) {
	if (isGbr) {
		return 1;
	} else {
		return 2;
...
void LteEnbRrc::SendSystemInformation() {
	...
	si.sib2.radioResourceConfigCommon.pdschConfigCommon.referenceSignalPower =
			m_cphySapProvider->GetReferenceSignalPower();
