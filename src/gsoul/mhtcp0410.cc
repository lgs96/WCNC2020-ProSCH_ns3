...
static const bool enablePcap = false;

//static const int cachePP = cacheOptimal;
static const int cachePP = evenPriority;
static const int cacheCP = AdditionalSendWhileCongestion;



static const int  numUEs = 1;
static const int  numNlosUEs = 1;
static const int  numExtraUEs = 0;
static const bool massiveSim = false;
static const bool allStationary = false;
static const bool mmwaveTrace = false;
static const bool needLog = false;
static const int  numOfBatch = 1;

double baseXPosition = 4;
double nonLosDuration = 3.5;
...
std::string nameBase = "BatchRtrans_off";
static const uint32_t  maxCacheLength = 65534*1000*0.1;
...
Vector ueStarting1 = Vector(3.5,0,1.5);
...
std::string dataRateArray[arraySize] = {"10Mbps", "30Mbps", "50Mbps", "100Mbps", "300Mbps"};
double dataRate_doubleArray[arraySize] = {10000000, 30000000, 50000000, 100000000, 300000000};
...
std::string delayArray[arraySize]={"50ms","50ms","50ms","50ms","50ms"};
double delayArrayDouble[arraySize]={ 0.05, 0.05, 0.05, 0.05, 0.05 };
...
main (int argc, char *argv[])
{
  ...
  if(!enableEA)
  {
	  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f_los%.2f",
			  nameBase.c_str(), "Conv", enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed,nonLosDuration,losDuration);
  }else
  {
	  switch(cachePP){
	  ...
	  {
		  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f_los%.2f",
				  nameBase.c_str(), "eP" ,enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed, nonLosDuration, losDuration);
		  break;
	  }
	  ...
	  {
		  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f_los%.2f",
				  nameBase.c_str(), "lF" ,enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed, nonLosDuration, losDuration);
		  break;
	  }
	  case nlosFirst:
	  {
		  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f_los%.2f",
				  nameBase.c_str(), "nF" , enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed, nonLosDuration, losDuration);
		  break;
	  }
	  ...
	  {
		  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f_los%.2f",
				  nameBase.c_str(), "PP" , enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed, nonLosDuration, losDuration);
		  break;
	  }

	  case cacheOptimal:
	  {
		  sprintf(fname, "%s_%s_%s_C%dk_RW%dM_d%dm_%.2fmPs_nlos%.2f",
				  nameBase.c_str(), "Opt" , enableCC?"CC":"NC" , maxCachesSize/1000 , rcvBufSize/1000000, distance, ueSpeed,nonLosDuration);
		  break;
	  }
	  }
  }
