...
public:
  ...
  virtual void DoSendProxyHoldRequest(uint32_t teid, double delay) = 0;
...
public:
  ...
  virtual void DoSendProxyHoldRequest(uint32_t teid,double delay);
...
// Process8 //
template <class C>
void MemberEpcEnbS1SapProvider<C>::DoSendProxyForwardingRequest (uint32_t seq, double delay, double interval)
{
  m_owner->DoProxyForwardingRequest(seq, delay, interval);
}

template <class C>
void MemberEpcEnbS1SapProvider<C>::DoSendProxyHoldRequest (uint32_t teid, double delay)
{
  m_owner->DoProxyHoldRequest(teid,delay);
}

template <class C>
void MemberEpcEnbS1SapProvider<C>::DoSendProxyReleaseRequest ()
{
  m_owner->DoProxyReleaseRequest();
}
//////////////
