...
//190225 added by gsoul for x2 end marker packet's header
EpcX2EndMarkerHeader::EpcX2EndMarkerHeader ()
  : m_numberOfIes (1),
	m_headerLength (4),
	m_gtpTeid (0xfffffffa)
{
...
TypeId
EpcX2EndMarkerHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcX2EndMarkerHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor<EpcX2EndMarkerHeader> ()
  ;
  return tid;
}

TypeId
EpcX2EndMarkerHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
EpcX2EndMarkerHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void
EpcX2EndMarkerHeader::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
...
uint32_t
EpcX2EndMarkerHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
...
void
EpcX2EndMarkerHeader::Print (std::ostream &os) const
{
  os << " gtpTeid=" << (uint32_t) m_gtpTeid;
  os << " LengthOfHeader=" << (uint32_t) m_headerLength;
  os << " NumberOfIEs=" << (uint32_t) m_numberOfIes;
}

uint32_t
EpcX2EndMarkerHeader::GetGtpTeid () const
{
  return m_gtpTeid;
}

void
EpcX2EndMarkerHeader::SetGtpTeid (uint32_t gtpTeid)
{
  m_gtpTeid = gtpTeid;
}

uint32_t
EpcX2EndMarkerHeader::GetLengthOfIes () const
{
  return m_headerLength;
}

uint32_t
EpcX2EndMarkerHeader::GetNumberOfIes () const
{
  return m_numberOfIes;
}
