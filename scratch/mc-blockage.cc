...
static void MakeLOS(double interval);
static void MakeNLOS(double interval);


	static void
MakeLOS (double interval)
{
	std::cout<<"LOS"<<std::endl;
	Config::SetDefault("ns3::MmWavePropagationLossModel::ChannelStates",StringValue("l"));
	Simulator::Schedule (Seconds(interval),&MakeNLOS,interval);
}
	static void
MakeNLOS (double interval)
{
	std::cout<<"NLOS"<<std::endl;
	Config::SetDefault("ns3::MmWavePropagationLossModel::ChannelStates",StringValue("n"));
	Simulator::Schedule (Seconds(interval),&MakeLOS,interval);
}
...
Traces(uint16_t nodeNum)
{
	...
	fileSST<<"tcp_sst_ue"<<nodeNum+1<<".txt";
...
//	LogComponentEnable("MmWavePropagationLossModel",LOG_LEVEL_DEBUG);
	...
	//LogComponentEnable("BuildingsObstaclePropagationLossModel", LOG_LEVEL_LOGIC);
	...
	double x2Latency = 0.00, mmeLatency=0.0;
...
//	Config::SetDefault ("ns3::MmWaveSpectrumPhy::DisableInterference",BooleanValue(true));
	...
	Config::SetDefault ("ns3::MmWavePhyMacCommon::ChunkWidth",DoubleValue(13.889e6/5));//1000MHz bandwidth
	...
	Config::SetDefault ("ns3::MmWavePointToPointEpcHelper::X2LinkDelay", TimeValue (Seconds(x2Latency)));
...
//	mmWaveEnbNodes.Add(mmWave73GhzEnbNodes);
	...
	Vector mmw1_1Position = Vector(0, 0, 10);  ///28Ghz
//	Vector mmw1_2Position = Vector(300, 300, 35); //28Ghz_2 
//	Vector mmw1_3Position = Vector(100,100,35); //28Ghz_3
//	Vector mmw2_1Position = Vector(-300,-300,35); //28Ghz_4
//	Vector mmw2_2Position = Vector(300,-300,35); //28Ghz_5
//	Vector mmw2_3Position = Vector(100,-100,35); //28Ghz_6
//	Vector mmw_73Position = Vector(0,0,35); //73Ghz
...
//	enbPositionAlloc->Add(mmw_73Position);
	...
	for (uint16_t i = 0; i<ueNodes.GetN(); i++) {
		if (i == 0)
			uePositionAlloc->Add(Vector(500, 0, 1.5));
		else if (i == 1)
			...
			uePositionAlloc->Add(Vector(0, -100, 1.5));
		else if (i == 5)
...
//	uemobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
	...
	building1->SetBoundaries(Box(460, 480,
				0,2, 
				0.0, 15));

	Ptr < Building > building2;
	building2 = Create<Building>();
	building2->SetBoundaries(Box(460, 480,
				6, 8,
				0.0, 15));
	
	   Ptr < Building > building3;
	   building3 = Create<Building> ();
	   building3->SetBoundaries (Box (460,480,
	   14, 16,
	   0.0, 10));
	 				                       /*
	...
	if (tcp){
		...
		for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
		{
			if (dl)
			{
				...
				app->Setup (ns3TcpSocket, sinkAddress, 1400, 0xffffffff, DataRate ("250Mbps"),isRandom);

				remoteHostContainer.Get (u)->AddApplication (app);

				double ueStartTime = 0.2;
				double ueGapTime = 0;
				double NLOSinterval = 0.5;
				...
				fileName_3<<"tcp_ack_throughput_ue_gsoul" << u+1 <<".txt";
				...
				Simulator::Schedule (Seconds(ueStartTime+u*ueGapTime+NLOSinterval),&MakeNLOS,NLOSinterval);
